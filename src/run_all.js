...
const MAX_PER_STORE = Number(process.env.MAX_POSTS_PER_STORE || 3);
const DAYS_TTL = Number(process.env.DAYS_TTL || 3);
const RATE_LIMIT_MS = Number(process.env.RATE_LIMIT_MS || 4500);
...
const stores = [
  ...
  { tag: "COLES", name: "Coles", hashtag: "#Coles", fn: fetchColes },
  { tag: "BIGW", name: "Big W", hashtag: "#BigW", fn: fetchBigW },
  { tag: "CHEMIST", name: "Chemist Warehouse", hashtag: "#ChemistWarehouse", fn: fetchChemistWarehouse },
  { tag: "JBHIFI", name: "JB Hi-Fi", hashtag: "#JBHiFi", fn: fetchJBHiFi },
];
...
for (const s of stores) {
  try {
    const deals = await s.fn({ limit: 20 });
    for (const d of deals) {
      const clean = sanitizePrices({ now: d.now, was: d.was });
      all.push({
        ...d,
        store: s.name,
        storeTag: s.tag,
        ...
        now: clean.now,
        was: clean.was,
        discountPct: calcDiscountPct(clean.now, clean.was),
      });
    }
  } catch {
...
let eligible = all.filter(
  d => d.title && d.url && d.now && !hasPosted(state, d)
);

// Rank deals
eligible.sort((a, b) => scoreDeal(b) - scoreDeal(a));
...
for (const d of eligible) {
  ...
  const count = perStore.get(d.storeTag) || 0;
  if (count >= MAX_PER_STORE) continue;
  perStore.set(d.storeTag, count + 1);
  selected.push(d);
}

// HARD fallback (text)
if (selected.length < MIN_DAILY) {
  const fallback = await fetchLocalFallback();
  for (const d of fallback) {
    if (selected.length >= MIN_DAILY) break;
    selected.push(d);
  }
}
...
for (const d of selected) {
  ...
  const caption = formatDealCard({
    ...
    was: d.was,
    discountPct: d.discountPct,
    hashtags: ["#TopDeals", d._hashtag, "#AustraliaDeals"].filter(Boolean),
  });
